...
# Bootstrap sys.path for the split-directory layout: this file lives in .../tu_muon/, but
...
# Colab runs `!python .../tu_muon_pipeline.py` directly.
    ...
    for attempt in range(max_retries):
        try:
            response = auto_model_relay.call_model(
                ...
                system_instruction=LLM_DISTRACTOR_SYSTEM_PROMPT, temperature=0.9, max_output_tokens=8192,
            )
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if args.command == "generate":
        ...
        generate(
            client, model, samples, Path(args.csv), Path(args.output),
